import React, { useState } from "react";
import { useForm } from "react-hook-form";
import { yupResolver } from "@hookform/resolvers/yup";
import * as yup from "yup";
import MkdSDK from "Utils/MkdSDK";
import { useNavigate, useParams } from "react-router-dom";
import { AuthContext, tokenExpireError } from "Context/Auth";
import { GlobalContext, showToast } from "Context/Global";

import { state_county_change } from "Utils/utils";
import { MkdInput } from "Components/MkdInput";
import { InteractiveButton } from "Components/InteractiveButton";
import { SkeletonLoader } from "Components/Skeleton";
import { FilterBoxBg } from "Components/FilterBoxBg";
import { SectionTitle } from "Components/SectionTitle";
import { ElectionTypeStateCountySelect } from "Components/AdminElectionComponents";
import TreeSDK from "Utils/TreeSDK";

const tdk = new TreeSDK();

const AdminEditPartyPage = () => {
  const { dispatch: globalDispatch } = React.useContext(GlobalContext);
  const schema = yup
    .object({
      name: yup.string().required(),
    })
    .required();

  const { dispatch, state } = React.useContext(AuthContext);
  const { partyId } = useParams();
  const [isSubmitLoading, setIsSubmitLoading] = React.useState(false);
  const [partyPicture, setPartyPicture] = React.useState({
    file: "",
    preview: "",
  });

  const [electionType, setElectionType] = useState([]);
  const [selected_states, setSelected_states] = useState([]);
  const [filtered_counties, setFiltered_counties] = useState([]);
  const [selected_county, setSelected_county] = useState([]);

  const [electionTypeErrorMessage, setElectionTypeErrorMessage] = useState("");
  const [stateErrorMessage, setStateErrorMessage] = useState(false);
  const [countyErrorMessage, setCountyErrorMessage] = useState(false);
  const [isFetching, setIsFetching] = useState(false);

  const navigate = useNavigate();
  const {
    register,
    handleSubmit,

    setError,
    setValue,
    formState: { errors },
  } = useForm({
    resolver: yupResolver(schema),
  });

  const selectImg = (e) => {
    if (e.target.files[0]) {
      setPartyPicture({
        file: e.target.files[0],
        preview: URL.createObjectURL(e.target.files[0]),
      });
    }
  };
  console.log(errors);
  const onSubmit = async (_data) => {
    let sdk = new MkdSDK();
    setIsSubmitLoading(true);
    try {
      // validation
      let isError = false;
      if (!electionType.length) {
        console.log("electionType error");
        setElectionTypeErrorMessage("Election type is required");
        isError = true;
      }
      if (!selected_states.length) {
        console.log("state error");
        setStateErrorMessage("State is required");
        isError = true;
      }
      if (!selected_county.length && electionType[0]?.value === 1) {
        console.log("county error");
        setCountyErrorMessage("County is required");
        isError = true;
      }
      if (isError) {
        setIsSubmitLoading(false);
        return;
      }

      let picture = "";
      if (partyPicture?.file) {
        const formData = new FormData();
        formData.append("file", partyPicture?.file);
        const uploadedResult = await sdk.uploadImage(formData);
        picture = uploadedResult?.url;
      } else {
        picture = partyPicture?.preview;
      }

      const result = await tdk.update("parties", partyId, {
        name: _data.name,
        logo: picture,
        state: selected_states[0]?.value,
        county: selected_county[0]?.value || null,
        election_type: electionType[0]?.value,
      });

      if (!result.error) {
        showToast(globalDispatch, "Party updated successfully.");
        navigate("/admin/party");
      }
      setIsSubmitLoading(false);
    } catch (error) {
      setIsSubmitLoading(false);
      console.log("Error", error);
      showToast(globalDispatch, error?.message, 4000, "error");
      tokenExpireError(dispatch, error.message);
    }
  };

  const getData = async () => {
    setIsFetching(true);
    try {
      const result = await tdk.getOne("parties", partyId);
      if (result.error || !result?.model?.id) {
        return showToast(globalDispatch, "Party not found", 4000, "error");
      }

      const { county, election_type, state, logo, name } = result?.model;
      // setValue("electionType", election_type);
      // setValue("county", county);
      // setValue("state", state);
      setValue("name", name);

      await state_county_change(
        [{ label: state, value: state }],
        setSelected_states,
        setFiltered_counties,
        setSelected_county,
        electionType
      );

      setSelected_states([{ label: state, value: state }]);
      setSelected_county([{ label: county, value: county }]);
      setElectionType([
        {
          value: election_type,
          label: Number(election_type) === 1 ? "County" : "State",
        },
      ]);
      setPartyPicture({
        ...partyPicture,
        preview: logo,
      });
    } catch (error) {
      console.log("Error", error);
      showToast(globalDispatch, error?.message, 4000, "error");
      tokenExpireError(dispatch, error?.message);
    }

    setIsFetching(false);
  };

  React.useEffect(() => {
    globalDispatch({
      type: "SETPATH",
      payload: {
        path: "party",
      },
    });

    getData();
  }, []);

  return (
    <div className=" p-10">
      <FilterBoxBg>
        {isFetching ? (
          <SkeletonLoader />
        ) : (
          <div>
            {" "}
            <SectionTitle
              className={"mb-8"}
              text={"Edit Party"}
              fontRoboto={true}
            />
            <form
              className=" w-full max-w-lg"
              style={{ fontFamily: "Inter, sans-serif" }}
              onSubmit={handleSubmit(onSubmit)}
            >
              <ElectionTypeStateCountySelect
                electionType={electionType}
                selected_county={selected_county}
                selected_states={selected_states}
                setElectionType={setElectionType}
                setSelected_county={setSelected_county}
                setSelected_states={setSelected_states}
                // setValue={setValue}
                electionTypeErrorMessage={electionTypeErrorMessage}
                setElectionTypeErrorMessage={setElectionTypeErrorMessage}
                stateErrorMessage={stateErrorMessage}
                setStateErrorMessage={setStateErrorMessage}
                countyErrorMessage={countyErrorMessage}
                setCountyErrorMessage={setCountyErrorMessage}
                filtered_counties={filtered_counties}
                setFiltered_counties={setFiltered_counties}
              />

              <MkdInput
                type={"text"}
                page={"parties"}
                name={"name"}
                errors={errors}
                label={"Name"}
                placeholder={"Name"}
                register={register}
                className={""}
              />

              <div className="">
                {" "}
                <img
                  src={partyPicture?.preview}
                  alt=""
                  className="mx-auto h-auto w-60 rounded-full"
                  loading="lazy"
                />
              </div>
              <div className="mb-8">
                <label className="mb-2 block  text-sm font-[400] ">
                  Select Party Logo
                </label>
                <input
                  className="block w-full cursor-pointer  rounded border border-transparent bg-[#f5f5f5] p-1  text-xs text-gray-900 file:rounded file:border-none file:bg-[#6b3099] file:px-2 file:py-2 file:text-white focus:outline-none"
                  type="file"
                  accept="image/*"
                  onChange={selectImg}
                />
              </div>

              <InteractiveButton
                type="submit"
                loading={isSubmitLoading}
                disabled={isSubmitLoading}
                className="rounded bg-gradient-to-tr from-[#662D91] to-[#8C3EC7] px-8 py-3 text-sm  font-[600] text-white hover:from-[#662D91] hover:to-[#662D91] disabled:cursor-not-allowed"
              >
                Submit
              </InteractiveButton>
            </form>
          </div>
        )}
      </FilterBoxBg>
    </div>
  );
};

export default AdminEditPartyPage;
