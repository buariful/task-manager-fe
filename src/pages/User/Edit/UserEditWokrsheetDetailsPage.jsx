...
export default function UserEditWokrsheetDetailsPage({}) {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      if (error) {
        ...
        navigate(`/user/edit-worksheet/${id}`);
      }
    } catch (error) {
      console.log(error?.message);
      showToast(
  ...
  return (
    <PageWrapper>
      <DetailPageHeader
        backLink={`/user/edit-worksheet/${id}`}
        pageTitle={"Edit Worksheet Details"}
        cancelBtnText="Cancel"
        cancelFunction={() => navigate(`/user/edit-worksheet/${id}`)}
        isLoading={isLoading}
