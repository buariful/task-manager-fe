...
export default function UserEditLevelDetailsPage() {
  ...
  const [skillPermissions, setSkillPermissions] = useState([]);
    ...
    try {
      ...
      const [
        ...
        supabase
          ...
          .eq("organization_id", state?.organization_id)
          .eq("status", "active"),
        supabase.from("level").select("*").eq("id", id).single(),
        fetchSinglePermission(state?.organization_id, "level", state?.role_id),
      ]);
      ...
      const modifiedLevels = levels?.map((l) => ({
        label: l.name,
        value: l.id,
      }));
      const modifiedTemplates = reportCardTemplates?.map((t) => ({
        label: t.name,
        value: t.id,
      }));

      // Destructure single level
      const {
        name,
        description,
        min_percentage,
  ...
  return (
    <PageWrapper>
      <DetailPageHeader
        backLink={`/user/edit-level/${id}`}
        pageTitle={"Edit Level Details"}
        cancelFunction={() => navigate(`/user/edit-level/${id}`)}
        isLoading={isLoading}
        submitFunction={levelPermissions?.add ? handleFormSubmit : () => {}}
      />
            ...
            <form ref={formRef} onSubmit={handleSubmit(onSubmit)}>
              <div className="grid grid-cols-2 gap-10">
                <div>
                  ...
                  <div className="mb-10">
                    <div className="mb-5 flex justify-between items-center gap-5">
                      ...
                      {levelPermissions?.document_upload ? (
                        <>
                          <button
                ...
                <div>
                  ...
                  {levelPermissions?.enable_disable ? (
                    <ToggleButton
                      withLabel={true}
                      value={isActive}
