  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      const { data: userProfile } = await supabase
        .from("user_profile")
        .select("*, organization(name, logo, login_page_image)")
        .eq("user_id", id)
        .single();
      ...
      //
      if (status !== "active") {
        showToast(globalDispatch, "You are not an active user.", 4000, "error");
        setSubmitLoading(false);
        return;
      }
      ...
      navigate(redirect_uri || "/administrator/dashboard");
    } catch (error) {
