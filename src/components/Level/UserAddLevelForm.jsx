...
export default function UserAddLevelForm({}) {
  ...
  const { data: skillPermission } = usePermissionFetcher("skill");
  ...
  const onSubmit = async (data) => {
    ...
    if (!permission?.add) {
      showToast(
        globalDispatch,
        "Permission denied for creating level.",
        4000,
        "error"
      );
      return;
  ...
  // const getData = async () => {
  //   try {
  //     const { data, error } = await fetchSinglePermission(
    ...
    <div className="">
      ...
      <div className="flex  justify-between items-center mb-6">
        <div className="flex items-center gap-2">
          <Link to="/user/level" className="text-accent">
            <FaArrowLeft />
          </Link>
      ...
      {permission?.add ? (
        permissionLoading ? (
          ...
          <div className=" p-10">
            ...
            >
              <AddLevelLeftSideForm
                errors={errors}
                register={register}
                documentUpload={permission?.document_upload}
              />
            </form>
            <div className={`${selectedTab === 2 ? "block" : "hidden"}`}>
              <LevelSkills hasAddSkillPermission={skillPermission?.add} />
            </div>
          </div>
        )
      ) : (
