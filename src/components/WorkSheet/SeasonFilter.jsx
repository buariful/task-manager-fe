...
export default function SeasonFilter({ visible, filters, setFilters }) {
  ...
  const handleLevelToggle = (e, value) => {
    const currentValues = filters.seasonIds || [];
    const newValues = e.target.checked
      ? [...currentValues, value]
      : currentValues.filter((v) => v !== value);

    setFilters((prev) => ({ ...prev, seasonIds: newValues }));
  };
          ...
          {loading ? (
            ...
            seasons?.map((option) => (
              ...
              >
                <input
                  type="checkbox"
                  checked={filters?.seasonIds?.includes(option.value) || false}
